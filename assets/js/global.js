...
// Set up the Intersection Observer to detect when elements are in view
const observer = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            entry.target.classList.add('show'); // Add the 'show' class to smoothly display
            observer.unobserve(entry.target); // Stop observing once it's visible
        }
    });
});

// Target all elements with the class 'content'
document.querySelectorAll('.content').forEach(content => {
    observer.observe(content); // Observe each content section
});